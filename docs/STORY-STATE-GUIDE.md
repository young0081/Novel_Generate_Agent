...
旧版本可能将 `world.rules` 保存为字符串数组，或将 `characters` 保存为“角色描述”字符串映射，例如 `"characters": {"master": "师尊，身合天道的大能"}`。当前版本会在加载时自动转换为带稳定 ID 的 `WorldRule` / `CharacterState` 对象，不需要用户手动编辑文件；角色描述会保留为 `current_status`，并从第一个标点前提取角色名。后续保存会使用结构化格式。无法识别的字段仍会报告具体文件错误，避免静默丢失设定。
