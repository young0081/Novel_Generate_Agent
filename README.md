# Novel Generate Team（团队协作同人小说 AI 创作平台）
...
这是一个**生产级 AI 协作创作平台**，核心是用 Rust 从零构建的 **AI Agent 运行时**（类似 Claude Code 的引擎）。它不仅能让 AI 写小说，更重要的是展示了如何构建一个**安全、可靠、可扩展的 AI Agent 系统**。
...
- **协作功能**：文学 git（分支/diff/回滚）+ 多供应商模型管理 + 会话续写
